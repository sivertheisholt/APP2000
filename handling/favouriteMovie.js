...
async function checkIfFavorited(movieId, user) {
    ...
    for(const movie of user.movieFavourites) {
        if(movie == movieId) {
            ...
            return new ValidationHandler(true, `Already favourited`);
        }
    }
...
async function addFavourite(movieId, userId) {
    ...
    if(isFavorited.status)
        return new ValidationHandler(true, 'Movie is already in favourites');
    const isSaved = await checkIfSaved(movieId);
    if(isSaved.status)
        return new ValidationHandler(true, 'Movie is saved');
